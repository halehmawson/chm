...
Then run the NMDS itself
```{r NMDS}
set.seed(42) #same result every run
#random permutations would give slightly different results each time
...
You can create a clean plot where the distance between points corresponds (more or less) to the degree of difference between rows/plots/sites/insert replicate type here.
```{r plot NMDS}
...
A PERMANOVA is structured the same as ANOVA, but relies on random permutations to test significance. There are more adjustable bells and whistles in the basic `adonis2` function in `vegan`, such as controlling the number of permutations, but these are the main components.
```{r basic permanova}
set.seed(42) #again, set seed to ensure your results are the same each time
per.results <- adonis2(dat.dist ~ Treatment * Factor, data = dat.resp)
per.results
```
